...
/* Power profile selection override */
typedef enum _meshtastic_Config_PowerConfig_ProfileOverride {
    /* Automatic profile selection based on power source (USB vs battery) */
    meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_AUTO = 0,
    /* Always use the plugged-in profile regardless of power source
 Useful when using battery packs that appear as USB power */
    meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_ALWAYS_PLUGGED = 1,
    /* Always use the battery profile regardless of power source
 Useful for maximum power savings even when plugged in */
    meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_ALWAYS_BATTERY = 2
} meshtastic_Config_PowerConfig_ProfileOverride;

/* Maximum power state to enter */
typedef enum _meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState {
    /* Allow all power states including SDS (LoRa radio off) */
    meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MAX_SDS = 0,
    /* Maximum state is Light Sleep (LoRa radio stays on) */
    meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MAX_LIGHT_SLEEP = 1,
    /* Maximum state is No Bluetooth (screen off, BT off, LoRa on, CPU active) */
    meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MAX_NO_BLUETOOTH = 2,
    /* Maximum state is Dark (screen off, BT on, LoRa on, CPU active) */
    meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MAX_DARK = 3,
    /* Stay in ON/POWER state (no power saving) */
...
typedef struct _meshtastic_Config_PowerConfig_PowerProfile {
 ...
 When false, device will never turn off LoRa radio, maintaining mesh connectivity */
    bool allow_deep_sleep;
    /* Allow the device to enter Light Sleep where CPU is suspended but LoRa stays on
 When false, CPU remains active for immediate mesh response */
    bool allow_light_sleep;
    /* Keep Bluetooth enabled in power saving states
 When false, Bluetooth is turned off to save power */
    bool bluetooth_enabled;
    /* Keep WiFi enabled in power saving states (if WiFi is configured)
 When false, WiFi is turned off to save power */
    bool wifi_enabled;
    /* Allow screen to wake up when packets arrive (messages, node updates, etc.)
 When false, screen stays off for incoming packets, saving power */
    bool screen_stays_responsive;
    /* Keep GPS enabled in power saving states
 When false, GPS may be turned off between position updates */
    bool gps_enabled;
    /* Screen timeout in seconds before turning off display
 0 uses system default, UINT32_MAX keeps screen always on */
    uint32_t screen_timeout_secs;
    /* Bluetooth timeout in seconds before turning off BT in power saving states
 0 uses system default */
    uint32_t bluetooth_timeout_secs;
    /* Minimum wake time in seconds after LoRa activity before entering light sleep
 0 uses system default */
...
typedef struct _meshtastic_Config_PowerConfig {
 ...
 Technical Details: ESP32 Only 0 for default of 10 seconds */
    ...
    /* Enhanced Power Management - Granular power profiles for different power sources
 When enabled, provides fine-grained control over power management behavior */
    bool use_granular_power_management;
    /* Override automatic profile selection */
    meshtastic_Config_PowerConfig_ProfileOverride force_profile;
    /* Power profile for when device is connected to external power (USB/DC) */
    bool has_plugged_in_profile;
    meshtastic_Config_PowerConfig_PowerProfile plugged_in_profile;
    /* Power profile for when device is running on battery power */
    bool has_battery_profile;
    meshtastic_Config_PowerConfig_PowerProfile battery_profile;
...
#define _meshtastic_Config_PowerConfig_ProfileOverride_MIN meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_AUTO
#define _meshtastic_Config_PowerConfig_ProfileOverride_MAX meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_ALWAYS_BATTERY
#define _meshtastic_Config_PowerConfig_ProfileOverride_ARRAYSIZE ((meshtastic_Config_PowerConfig_ProfileOverride)(meshtastic_Config_PowerConfig_ProfileOverride_PROFILE_ALWAYS_BATTERY+1))

#define _meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MIN meshtastic_Config_PowerConfig_PowerProfile_MaxPowerState_MAX_SDS
...
#define meshtastic_Config_PowerConfig_PowerProfile_wifi_enabled_tag 4
#define meshtastic_Config_PowerConfig_PowerProfile_screen_stays_responsive_tag 5
#define meshtastic_Config_PowerConfig_PowerProfile_gps_enabled_tag 6
#define meshtastic_Config_PowerConfig_PowerProfile_screen_timeout_secs_tag 7
...
#define meshtastic_Config_PowerConfig_use_granular_power_management_tag 10
#define meshtastic_Config_PowerConfig_force_profile_tag 11
#define meshtastic_Config_PowerConfig_plugged_in_profile_tag 12
#define meshtastic_Config_PowerConfig_battery_profile_tag 13
...
X(a, STATIC,   SINGULAR, BOOL,     use_granular_power_management,  10) \
X(a, STATIC,   SINGULAR, UENUM,    force_profile,    11) \
X(a, STATIC,   OPTIONAL, MESSAGE,  plugged_in_profile,  12) \
X(a, STATIC,   OPTIONAL, MESSAGE,  battery_profile,  13) \
...
#define meshtastic_Config_PowerConfig_plugged_in_profile_MSGTYPE meshtastic_Config_PowerConfig_PowerProfile
#define meshtastic_Config_PowerConfig_battery_profile_MSGTYPE meshtastic_Config_PowerConfig_PowerProfile

#define meshtastic_Config_PowerConfig_PowerProfile_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, BOOL,     allow_deep_sleep,   1) \
X(a, STATIC,   SINGULAR, BOOL,     allow_light_sleep,   2) \
X(a, STATIC,   SINGULAR, BOOL,     bluetooth_enabled,   3) \
X(a, STATIC,   SINGULAR, BOOL,     wifi_enabled,      4) \
X(a, STATIC,   SINGULAR, BOOL,     screen_stays_responsive,   5) \
X(a, STATIC,   SINGULAR, BOOL,     gps_enabled,       6) \
